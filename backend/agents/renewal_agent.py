"""
ADIP — Renewal Agent (Reasoning Layer)

Analyzes contract renewal readiness and produces renewal-specific recommendations.
"""
from __future__ import annotations

from pathlib import Path

from backend.agents.base_agent import BaseAgent
from backend.core.shared_state import ADIPState, AgentResult
from backend.services.gemini_service import GeminiService

PROMPT_PATH = Path(__file__).parent.parent / "prompts" / "renewal_agent.txt"


class RenewalAgent(BaseAgent):
    name = "RenewalAgent"
    description = "Analyzes contract renewal readiness and renewal risk."
    category = "reasoning"
    capabilities = ["analyze_renewal"]
    priority = 40

    def __init__(self) -> None:
        super().__init__()
        self.gemini = GeminiService()
        self._prompt_template = PROMPT_PATH.read_text(encoding="utf-8")

    async def execute(self, state: ADIPState) -> AgentResult:
        customer = state.customer
        retrieved_context = "\n".join(
            f"- [{c.document_type}] {c.source}: {c.content[:200]}"
            for c in state.retrieved_chunks[:5]
        ) or "No context retrieved."

        memory_context = "\n".join(
            f"- {m.get('recommendation', '')} → {m.get('decision', '')}"
            for m in state.memory_context
        ) or "No previous decisions."

        # Calculate days to renewal
        from datetime import datetime, date
        days_to_renewal = None
        try:
            renewal = datetime.strptime(customer.renewal_date, "%Y-%m-%d").date()
            days_to_renewal = (renewal - date.today()).days
        except Exception:
            pass

        prompt = self._prompt_template.format(
            company_name=customer.company_name,
            health_score=customer.health_score,
            renewal_date=customer.renewal_date,
            active_users=customer.active_users,
            licensed_users=customer.licensed_users,
            contract_value=customer.contract_value,
            champion_status=customer.champion_status,
            months_active=customer.months_active,
            detected_signals=", ".join(state.detected_signals) or "none",
            retrieved_context=retrieved_context,
            memory_context=memory_context,
        )

        fallback = {
            "renewal_likelihood": "medium",
            "renewal_probability": 0.5,
            "days_to_renewal": days_to_renewal,
            "key_renewal_risks": ["Insufficient data for renewal assessment"],
            "key_renewal_strengths": [],
            "reasoning": "Renewal analysis could not be completed at this time.",
            "confidence_inputs": {"signal_strength": 0.3, "evidence_quality": 0.3},
            "recommended_actions": [],
        }

        result = await self.gemini.generate_json(prompt, fallback=fallback)
        ci = result.get("confidence_inputs", {})
        confidence = (ci.get("signal_strength", 0.3) + ci.get("evidence_quality", 0.3)) / 2

        self.logger.info(
            f"Renewal likelihood: {result.get('renewal_likelihood')} | "
            f"days_to_renewal: {days_to_renewal}"
        )

        return AgentResult(
            agent_name=self.name,
            status="success",
            confidence=confidence,
            reasoning=result.get("reasoning", ""),
            evidence=result.get("key_renewal_risks", []) + result.get("key_renewal_strengths", []),
            recommendations=result.get("recommended_actions", []),
            metadata={
                "renewal_likelihood": result.get("renewal_likelihood"),
                "renewal_probability": result.get("renewal_probability"),
                "days_to_renewal": days_to_renewal,
            },
        )
